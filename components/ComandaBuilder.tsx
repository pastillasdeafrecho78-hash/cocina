...
export interface ModificadorCategoria {
  id: string
  modificadorId: string
  modificador: Modificador
}
...
export type ComandaCheckoutItem = {
  productoId: string
  tamanoId?: string
  cantidad: number
...
export type ComandaBuilderProps = {
  categorias: Categoria[]
  mesas?: Mesa[]
  initialMesaId?: string
  comandaIdParam?: string | null
  /** Staff: tipo de pedido + selector de mesa. Público: oculto (el tipo lo define la página). */
  showStaffTipoMesaControls?: boolean
  kicker?: string
  title: string
  subtitle?: string
  backButton?: ReactNode
  /** Contenido extra encima del botón principal (ej. datos del cliente en pedido público). */
  ...
  }) => Promise<{ ok: boolean; error?: string; redirectTo?: string; message?: string }>
}

export function ComandaBuilder({
  categorias,
  mesas = [],
  initialMesaId = '',
  comandaIdParam = null,
  showStaffTipoMesaControls = true,
  kicker = 'Comandas',
  title,
  subtitle,
  backButton,
  ...
  observacionesPlaceholder = 'Alergias, instrucciones generales...',
  loading = false,
  onCheckout,
}: ComandaBuilderProps) {
  ...
  const handleGuardar = async () => {
    ...
    setGuardando(true)
    try {
      const itemsPayload: ComandaCheckoutItem[] = carrito.map((item) => ({
      ...
      const data = await onCheckout({
        items: itemsPayload,
        observaciones,
        mesaId: showStaffTipoMesaControls ? mesaId || undefined : undefined,
        tipoPedido: showStaffTipoMesaControls ? tipoPedido : undefined,
        comandaId: comandaIdParam,
      })
      if (data.ok) {
        const defaultMsg = comandaIdParam
          ? 'Pedidos agregados'
          : showStaffTipoMesaControls
        ...
        toast.error(data.error || 'No se pudo completar el pedido')
      }
    } catch {
      toast.error(comandaIdParam ? 'Error al agregar pedidos' : 'Error al enviar el pedido')
    } finally {
      setGuardando(false)
    }
  }
  ...
  return (
    <div className="app-page">
      ...
      <div className="app-card mb-6">
        ...
        <h1 className="mt-2 text-3xl font-semibold text-stone-900 dark:text-stone-100">{title}</h1>
        {subtitle ? <p className="mt-1 text-stone-600 dark:text-stone-400">{subtitle}</p> : null}
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        {/* ── PANEL IZQUIERDO ───────────────────────────────────────────── */}
        <div className="lg:col-span-2">
          {showStaffTipoMesaControls && !esAgregarPedidos && (
            <>
          <div className="mb-4">
        ...
        <div className="lg:col-span-1">
          <div className="app-card sticky top-4 p-6">
            ...
            <div className="mb-4">
              <label className="mb-2 block text-sm font-medium text-stone-700">{observacionesLabel}</label>
              <textarea
                value={observaciones}
            ...
            >
              {guardando
                ? 'Guardando...'
                : submitLabel ??
                  (esAgregarPedidos ? 'Agregar pedidos' : showStaffTipoMesaControls ? 'Crear Comanda' : 'Confirmar pedido')}
            </button>
          </div>
        </div>
      </div>
